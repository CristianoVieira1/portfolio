        ...
        <div ref={innerRef} className="inner">
          <div className="header-top">
            ...
            <div className="close-menu">
              <button className="close-button tmp-round-action-btn" onClick={() => closeModal("mobileMenu1")}>
            ...
            {menuItems.map((item, index) => (
              <li
                ...
                className={`${item.hasDropdown ? "has-dropdown" : ""} ${
                  item.submenu?.some((elm) => elm.href.split("/")[1] == pathname.split("/")[1]) ? "menu-item-open" : ""
                }`}
              >
                {item.isLink ? (
                  <Link className={`${item.href.split("/")[1] == pathname.split("/")[1] ? "active" : ""}`} to={item.href}>
                    {item.label}
                  </Link>
                ) : (
                  <a
                ...
                {item.hasDropdown && (
                  <ul className="submenu" style={{ display: activeParent == index ? "block" : "none" }}>
                    {item.submenu.map((subItem, subIndex) => (
                      <li key={subIndex}>
                        <Link className={`${subItem.href.split("/")[1] == pathname.split("/")[1] ? "active" : ""}`} to={subItem.href}>
                          {subItem.label}
                        </Link>
                      </li>
                    ))}
